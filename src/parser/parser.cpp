...
namespace HLang
{
    PExprNode ExprParser::parse(int precedence) {
        ...
        if (prefixParseMap.find(token.Type) == prefixParseMap.end()) {
            throw ParseError("Expected expression, but '" + token.Value + "' found",
    ...
    void ExprParser::registerParser() {
        ...
        registerLiteral(TokenType::DoubleQuote); // string

        registerPrefix(TokenType::Identifier, make_unique<IdentifierParse>());
        ...
        registerInfix(TokenType::Quest, make_unique<TernaryOperatorParse>());
        ...
        // Group
        registerPrefix(TokenType::LeftParen, make_unique<ParenthesesParse>()); // Ex: -2, -3

        // Assignment
        registerInfix(TokenType::Assign, make_unique<AssignmentParse>());
    }
    ...
    void ExprParser::registerLiteral(TokenType type) {
        registerPrefix(type, make_unique<LiteralParse>());
    }

    void ExprParser::registerUnary(TokenType type, Precedence precedence) {
        registerPrefix(type, make_unique<UnaryOperatorParse>(precedence));
    }

    void ExprParser::registerBinary(TokenType type, Precedence precedence) {
        registerInfix(type, make_unique<BinaryOperatorParse>(precedence, false));
    }

    void ExprParser::registerBinaryRightAssociative(TokenType type, Precedence precedence) {
        registerInfix(type, make_unique<BinaryOperatorParse>(precedence, true));
    }
}